...
// Check, if username session is NOT set then this page will jump to login page
  if (!isset($_SESSION['login_user'])) {        //만약 로그인되어 있지 않은 상태라면 로그인 상태로 이동
...
<body>
  ...
  <hr>
    ...
    <button id="Rbutton" type="button" onclick="addvideo()">카메라 추가</button>
    <button id="Rbutton" type="button" onclick="removevideo()">카메라 삭제</button>
    <button id="Rbutton" type="button" onclick="removeALLvideo()">카메라 전체 삭제</button>
...
function setcamera(name,x,y){                     //add camera에서 돌아온 후 이 함수 실행, 입력받은 정보를 토대로 set
  listname[pnum]=name;
  CameraLocation=name;
  CameraX=x;
  CameraY=y;
...
function addvideo(){
  ...
  // window.name = "부모창 이름";
            ...
            // window.open("open할 window", "자식창 이름", "팝업창 옵션");
            openWin = window.open("mapadd2.html",                    //새 창으로 띄움
                    "AddMap", "width=200, height=250, resizable = no, scrollbars = no");
}

function removevideo(){         //선택 카메라 삭제
  if(OnOff==false)
  {
    ...
    if( retVal == true ){
      ...
      markers[selectmarker].setMap(null);                       //삭제시 마커, 리스너, 위치, 정보창, 리스트네임 전부 다 삭제
...
function removeALLvideo()             //전체 카메라 삭제
{
